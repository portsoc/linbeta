...
/* ---- Setting up and doing ONLOAD ---- */
	...
	h.onchange = function(event){ flip('help') }
	a.onchange = function(event){ flip('add'); }
	l.onchange = function(event){ flip('links') }

	// Set up the "submit to add an entry" event handler
	var p = document.getElementById("postage")
	p.onsubmit = function(event) { InsertEntry(); return false; };
...
/* ---- Turning display of the a part on/off ---- */
    ...
    function flip(here)
      { var elem = document.getElementById(here).style;
...
/* ---- POSTing a form entry, ultimately to ENTRY.INSERT.AJAX.PHP ----*/
	...
	{ var p=document.getElementById("postage")
	  var c = p.cat.value; if (c=="") c = "?";
	  var u = p.url.value;
	  var k = p.cap.value; if (k=="") k = u;
	  // alert("cat/url/cap = /"+c+"/"+u+"/"+k+"/")
      	  var x = { cat:c, url:u, cap:k }
	  if (logging)
