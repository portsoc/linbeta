...
window.linora.keys = (function () {
	...
	var
		keys = { tab: 9, enter: 13, esc: 27, left: 37, up: 38, right: 39, down: 40 },
		...
		doselect = function(e) {
			...
			switch(e.keyCode) {
				...
				//case (keys.tab): window.linora.ui.panelAdvance(1); return false;
			}
			...
			if (document.getElementById("links.flip").checked) {
				...
				switch(e.keyCode) {
					case (keys.up): window.linora.ui.selectElem(false); e.preventDefault(); return false;
					case (keys.down): window.linora.ui.selectElem(true); e.preventDefault(); return false;
