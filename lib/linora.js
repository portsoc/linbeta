...
window.linora.ui = (function () {
    ...
    var
		...
		*/
        ...
        prepareResultList = function(where) {

            where.appendChild(document.createElement("ul")).classList.add("links");
        },
        ...
        updateSearch = function() {
            search.value = url.search.substring(1).split('&')[0].substring(2);
            doSearch();
        },
		...
		doSearch = function (e) {
			...
			window.linora.xhr.load(
				{
					"url": "api/2/links?filter="+search.value,
					"callbacks": {
						"load": function(e) {
							var result = JSON.parse(e.target.responseText); 
							replaceResults( result, "#links .links", true, true );
						},
						"error": failedToLoad
					}
				}
        ...
        setup = function () {
            prepPanelHandles();
            prepareResultList( n.links.panel );
            attachFlipListeners();
            disableFormSubmit();
            //refreshLinksPanel();
            updateSearch();
